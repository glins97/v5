    ...
    def leaf_checkbox_html(self):
        ...
        <div class="form-check col-sm">
            <label class="form-check-label">
                <input class="form-check-input" name="competencyError" type="checkbox" value="" id="checkbox-{code}" onclick="updateTextfieldValue('checkbox-{code}', '{target}', '{desc} ', 'c{competency}', {weight}, {apply})"> {name}
                <span class="form-check-sign">
                    <span class="check"></span>
                </span>
            </label>
        </div>
        """.format(code=self.get_verbose_code().replace('.', '-'), target=self.target, desc=self.description if self.description else '', name='{} {}'.format(self.get_verbose_code(), self.name), competency=self.competency, weight=self.weight, apply=('true' if self.apply_on_select else 'false')))
